...
- Kodlama fazı: 🟡 devam ediyor. Backend + n8n + panel + AI (Gemini) + Redis rate limit +
  ...
  Apache dev vhost'u + Postgres Windows servisi hepsi tamamlandı (PHASE_14-28). **PHASE_28'de
  gerçek bir Meta App/test WABA'sı da bağlandı** — token/tenant eşleşmesi gerçek Meta Graph
  API'ye karşı doğrulandı, ama fiili mesaj teslimatı **`131031 Business Account locked`**
  hatasına takıldı (Meta hesap düzeyinde kilit, kod hatası değil — kullanıcının Meta Business
  Suite'te çözmesi gerekiyor). Kalan büyük açık: **Meta hesap kilidinin kalkması** (madde 25'in
  geri kalanı: Embedded Signup + backend `/internal/whatsapp/send`'in gerçek kimlik bilgisiyle
  ilk testi) + n8n queue mode (madde 16, yalnızca ölçekleme aşamasında, dokunulmadı).

## Bu Oturumda Yapılanlar (PHASE_29 — Apache Authorization header bug'ı)
...
AI yönlendirmesi, reschedule ucu, Apache dev vhost'u, Postgres Windows servisi hepsi bitti.
**Tek büyük açık: Meta hesap kilidi (`131031 Business Account locked`, PHASE_28).** Öncelik:

1. **(Meta hesap kilidi kalkınca) Gerçek WABA ile ilk uçtan uca doğrulama** — dev tenant'ta
   gerçek kimlik bilgileri zaten yazılı (`phone_number_id=1215172075008488`,
   `waba_id=2753419422874528`, token **24 saatlik geçiciydi, süresi dolmuşsa Meta panelinden
   "Generate new token" ile tazelenip aynı yöntemle** `TokenCipher::encrypt` + `psql UPDATE
   tenants` **ile yeniden yazılmalı**). Kilit kalkınca: (a) doğrudan Meta Graph API testi
   (`curl --ssl-no-revoke`, PHASE_28'deki gibi), (b) backend'in kendi `POST
   /internal/whatsapp/send` ucu (henüz hiç gerçek kimlik bilgisiyle denenmedi), (c)
   `GET/POST /webhook/whatsapp` + `N8nNotifier` gerçek trafik doğrulaması, (d) PHASE_12'nin
   "geçersiz imza → 403, tenant bulunamadı → 200" kararının gözden geçirilmesi, (e) Embedded
   Signup panele kodlanması (BACKLOG §A madde 25).
2. **(Düşük öncelik, opsiyonel) n8n reschedule dalını tek çağrıya indir** — Backend ucu
...
3. n8n queue mode deposu (BACKLOG §A madde 16'nın kalan yarısı) — yalnızca gerçek ölçekleme
   ihtiyacı doğduğunda; tek instance n8n şu an yeterli.
4. FullCalendar ticari lisansı (BACKLOG §B E8) zaten kapalı (ücretsiz çekirdeğe geçildi,
